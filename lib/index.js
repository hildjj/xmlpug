(function() {
  ...
  this.cmd = function(args, cb) {
    ...
    program.version(pkg.version).usage('[options] <template> [input]').option('-c, --config <file>', "Config file to read [" + DEFAULT_CONFIG + "]", DEFAULT_CONFIG).option('-d, --debug', 'Add Jade debug information').option('--doublequote', 'Use doublequotes instead of single').option('-i, --xinclude', 'Process XInclude when parsing XML').option('-o, --output [file]', 'Output file').option('-p, --pretty', 'Pretty print').option('--html', 'HTML output; only useful for pretty printing').option('-s, --source [file]', 'Output source for client transformation').parse(args);
