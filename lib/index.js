(function() {
  var dentToString, fix, fs, jade, path, pkg, req, temp, transform, xml;
  ...
  this.transform = transform = function(jadedata, xmldata, options) {
    var cache, err, fn, out, pretty, xmldoc;
    if (options == null) {
      options = {
        pretty: true
      };
  ...
  this.cmd = function(args, cb) {
    ...
    program.version(pkg.version).usage('[options] <template> [input]').option('-d, --debug', 'Add Jade debug information').option('-o, --output [file]', 'Output file').option('-p, --pretty', 'Pretty print').option('--html', 'HTML output; only useful for pretty printing').option('-s, --source [file]', 'Output source for client transformation').parse(args);
