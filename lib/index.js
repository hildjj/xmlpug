 ...
 * @param {string|Document} xmldata The XML Document as source.
 * @param {XmlPugOptions} options Configuration.
 * @returns {string} The transformed document.
 */
export function transform(pugdata, xmldata, options) {
  options = {
    pretty: true,
    dentinColors: false,
    ...options,
  ...
  if (xmldata instanceof Document) {
    ...
    xmldoc = parseXmlString(xmldata, {
      noent: true,
      baseUrl: options.xmlFileName ?
        pathToFileURL(options.xmlFileName).toString() :
        undefined,
