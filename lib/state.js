 ...
 *
 * @param {null|string|number|xml.Text} r Source for normalization.
 * @returns {string|number|null|xml.Node} Normalized value.
 */
function fix(r) {
  if (r == null) {
    return null
  ...
  switch (x.type()) {
    case 'attribute':
      return /** @type xml.Attribute */(x).value()
    case 'text':
      return /** @type xml.Text */(x).text()
    default:
      return x
  }
}
